...
<i>Бот-помощник актива ПБ2 работает на открытом исходном коде, который размещён на портале "Github". За подробностями отправляйся по <a href="https://github.com/dan-sazonov/MagistrKurskBot">ссылке</a>.</i>
...
<b>План такой:</b>
1️1. напиши, что ты хочешь получить в подарок;
2. напиши, что ты точно НЕ хочешь получить в подарок (например, если на что-нибудь у тебя аллергия);
3. реши, сможешь ли ты приехать на встречу актива 18 декабря;
4. укажи, как тебя зовут и в каком из офисов тебе будет удобно забрать подарок;
5. затем через несколько дней наша система подберёт для тебя человека, которому ты сделаешь подарок!
...
(За более подробной информацией обратись к @dan_sazonov)'''
            self.ask_address = '''<b>Шаг 3.</b> В каком из офисов профкома тебе было бы удобно забрать подарок? Нажми кнопку, которая соответствует твоему решению.'''
