...
(За более подробной информацией обратись к @dan_sazonov)'''
            self.ask_address = '''<b>Шаг 3.</b> Если у тебя не получится прийти на встречу, в каком из офисов профкома тебе было бы удобно забрать подарок? Нажми кнопку, которая соответствует твоему решению.'''
